...
EDeviceType parseAppleDeviceType(uint8_t *pData, uint8_t len)
{
	// pData指向Apple Type后面的数据，len是长度
	// 根据你提供的广播包，Apple Type = 0x12，后面数据结构不公开完全，
	// 这里用简单经验判断：
	// 例如，某字节特定值代表键盘或鼠标，或者根据广播包特征判断	
	// 这里示例用第1字节判断（实际需根据具体协议调整）
	if(len < 1)
		return eUnknown;	
	uint8_t deviceFlag = pData[0];	
	// 经验判断：假设deviceFlag 0x2a代表键盘，0x2b代表鼠标（示例）
	// 你需要根据实际抓包分析调整
	if(deviceFlag == 0x2a)
		return eKeyboard;
	else if(deviceFlag == 0x2b)
		return eMouse;	
	return eUnknown;
}
...
uint8_t parseDeviceInfo(gapDeviceInfoEvent_t* info)
{
	...
	for(uint8_t i = 0; i < l; i)
	{
		...
		else if(type == GAP_ADTYPE_MANUFACTURER_SPECIFIC)	// 厂商自定义数据
		{
			uint16_t v = BUILD_UINT16(*(pVal+0), *(pVal+1));
			if(v == 0x004c)	// Apple Inc.
			{
				LOG("    厂商id Apple Inc. 0x004c\r\n");
				// 解析下 pVal剩余的数据
				uint8_t appleType = pVal[2];
				if(appleType == 0x12) // 苹果无线配件广播
				{
					LOG("    苹果无线设备\r\n");
					EDeviceType devType = parseAppleDeviceType(&pVal[4], siz - 4);
					if(devType == eKeyboard)
					{
						LOG("    设备类型: Keyboard\r\n");
						return eKeyboard;
					}
					else if(devType == eMouse)
					{
						LOG("    设备类型: Mouse\r\n");
						return eMouse;
					}
				...
				if(siz > 3)
				{
					uint8_t* pApple = pVal + 2;	// 跳过厂商ID
					uint8_t appleType = *pApple++;
					if(appleType == 0x02)	// Apple设备类型
					{
						uint16_t appleDevType = BUILD_UINT16(*pApple, *(pApple + 1));
						LOG("    Apple设备类型 0x%04X\r\n", appleDevType);
						if(appleDevType == 0x0212)	// Apple Keyboard
						{
							ret = eKeyboard;
						}
						else if(appleDevType == 0x0213)	// Apple Mouse
						{
							ret = eMouse;
						}
					}
			...
			else if (v == 0x046D)  // Logitech Inc.
			{
				LOG("    厂商id Logitech Inc. 0x046D\r\n");
			}
			else
			{
				LOG("    厂商id 0x%04X 未知\r\n", v);
	...
	switch(pEvent->gap.opcode)
	{
		...
		case GAP_DEVICE_INFO_EVENT:
		{
			if(pEvent->deviceInfo.rssi > requireRSSI)
			{
				...
				else if(eventType == GAP_ADRPT_ADV_NONCONN_IND)
				{
					LOG("  扫描到广播包 无定向广播 不可连接 \r\n");
					Print_Memory(p, l, 1);
				}
				else
				{
					LOG("  未知扫描包 eventType %d!!! \r\n", eventType);
				}
			}
