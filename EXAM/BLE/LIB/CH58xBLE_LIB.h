...
#define bleAlreadyInRequestedMode       0x11   // 设备已经启动  //!< Already performing that task
...
#define blePending                      0x16   // –当返回到客户端功能时，服务器或 GATT 子过程正在进行中，有待处理的响应。 –返回服务器功能时，来自客户端的确认待处理。  //!< Waiting
#define bleTimeout                      0x17   //!< Timed out performing function
#define bleInvalidRange                 0x18   // 参数超出范围  //!< A parameter is out of range
...
#define ATT_ERR_INSUFFICIENT_AUTHEN     0x05 // 该属性需要进行身份验证才能被读取或写入  //!< The attribute requires authentication before it can be read or written
#define ATT_ERR_UNSUPPORTED_REQ         0x06 // 属性服务器不支持从属性客户端收到的请求。  //!< Attribute server does not support the request received from the client
#define ATT_ERR_INVALID_OFFSET          0x07 // 指定的偏移量超出了属性的末尾。  //!< Offset specified was past the end of the attribute
#define ATT_ERR_INSUFFICIENT_AUTHOR     0x08 // 该属性需要授权才能被读取或写入。  //!< The attribute requires authorization before it can be read or written
#define ATT_ERR_PREPARE_QUEUE_FULL      0x09 // 准备写入的队列过多。  //!< Too many prepare writes have been queued
#define ATT_ERR_ATTR_NOT_FOUND          0x0a // 在给定的属性句柄范围内找不到属性。  //!< No attribute found within the given attribute handle range
#define ATT_ERR_ATTR_NOT_LONG           0x0b // 无法使用读取 Blob 请求或准备写入请求来读取或写入属性  //!< The attribute cannot be read using the Read Blob Request
#define ATT_ERR_INSUFFICIENT_KEY_SIZE   0x0c // 用于加密此链接的加密密钥大小不足。  //!< The Encryption Key Size used for encrypting this link is insufficient
#define ATT_ERR_INVALID_VALUE_SIZE      0x0d //!< The attribute value length is invalid for the operation
#define ATT_ERR_UNLIKELY                0x0e // 请求的属性请求遇到了一个不太可能发生的错误，并且未能按要求完成。  //!< The attribute request that was requested has encountered an error that was very unlikely, and therefore could not be completed as requested
#define ATT_ERR_INSUFFICIENT_ENCRYPT    0x0f // 该属性需要加密才能读取或写入。  //!< The attribute requires encryption before it can be read or written
#define ATT_ERR_UNSUPPORTED_GRP_TYPE    0x10 // 属性类型不是更高层规范定义的受支持的分组属性。  //!< The attribute type is not a supported grouping attribute as defined by a higher layer specification
#define ATT_ERR_INSUFFICIENT_RESOURCES  0x11 // 资源不足，无法完成请求。  //!< Insufficient Resources to complete the request
...
#define GAP_DEVICE_INIT_DONE_EVENT              0x00 // 当设备初始化完成置此事件。  //!< Sent when the Device Initialization is complete.  This event is sent as an tmos message defined as gapDeviceInitDoneEvent_t.
#define GAP_DEVICE_DISCOVERY_EVENT              0x01 // 设备发现过程完成时置此事件。  //!< Sent when the Device Discovery Process is complete. This event is sent as an tmos message defined as gapDevDiscEvent_t.
...
#define GAP_END_DISCOVERABLE_DONE_EVENT         0x04 // 当广播结束时置此事件。  //!< Sent when the Advertising has ended. This event is sent as an tmos message defined as gapEndDiscoverableRspEvent_t.
#define GAP_LINK_ESTABLISHED_EVENT              0x05 // 建立连接后置此事件。  //!< Sent when the Establish Link Request is complete. This event is sent as an tmos message defined as gapEstLinkReqEvent_t.
#define GAP_LINK_TERMINATED_EVENT               0x06 // 连接断开后置此事件。  //!< Sent when a connection was terminated. This event is sent as an tmos message defined as gapTerminateLinkEvent_t.
#define GAP_LINK_PARAM_UPDATE_EVENT             0x07 // 接收到参数更新事件后置此事件。  //!< Sent when an Update Parameters Event is received. This event is sent as an tmos message defined as gapLinkUpdateEvent_t.
...
#define GAP_DEVICE_INFO_EVENT                   0x0D // 在发现设备期间发现设备置此事件。  //!< Sent during the Device Discovery Process when a device is discovered. This event is sent as an tmos message defined as gapDeviceInfoEvent_t.
...
// Timers
#define TGAP_GEN_DISC_ADV_MIN                   0   // 通用广播模式的广播时长，单位:0.625ms（默认值：0）  //!< Minimum time to remain advertising, when in Discoverable mode.Default 0-turns off the timeout. (n * 0.625 mSec).
#define TGAP_LIM_ADV_TIMEOUT                    1   // 限时可发现广播模式的广播时长，单位:1s（默认值：180）  //!< Maximum time to remain advertising, when in Limited Discoverable mode.Default 180 seconds. (n * 1 seconds)
#define TGAP_DISC_SCAN                          2   // 扫描时长，单位:0.625ms（默认值：16384）  //!< Minimum time to perform scanning,Setting this parameter to 0 turns off the timeout.Default 10.24seconds. (n * 0.625 mSec)

// when in General Discovery process
#define TGAP_DISC_ADV_INT_MIN                   3   // 最小广播间隔，单位:0.625ms（默认值：160）  //!< Minimum advertising interval.Default 160. (n * 0.625 mSec)
#define TGAP_DISC_ADV_INT_MAX                   4   // 最大广播间隔，单位:0.625ms（默认值：160）  //!< Maximum advertising interval.Default 160. (n * 0.625 mSec)
#define TGAP_DISC_SCAN_INT                      5   // 扫描间隔，单位:0.625ms（默认值：16）  //!< Scan interval used during Link Layer Scanning state.Default 16. (n * 0.625 mSec)
#define TGAP_DISC_SCAN_WIND                     6   // 扫描间隔，单位:0.625ms（默认值：16）  //!< Scan window used during Link Layer Scanning state.Default 16. (n * 0.625 mSec)

// when in Connection Establishment process(1M PHY)
#define TGAP_CONN_EST_INT_MIN                   7   // 建立连接的最小连接间隔，单位:1.25ms（默认值：80）  //!< Minimum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_INT_MAX                   8   // 建立连接的最大连接间隔，单位:1.25ms（默认值：80）  //!< Maximum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_SCAN_INT                  9   // 建立连接的扫描间隔，单位:0.625ms（默认值：16）  //!< Scan interval used during Link Layer Initiating state.Default 16. (n * 0.625 mSec)
#define TGAP_CONN_EST_SCAN_WIND                 10  // 建立连接的扫描窗口，单位:0.625ms（默认值：16）  //!< Scan window used during Link Layer Initiating state.Default 16. (n * 0.625 mSec)
...
#define TGAP_CONN_EST_SUPERV_TIMEOUT            13  // 建立连接的连接管理超时时间，单位:10ms（默认值：2000）  //!< Link Layer connection supervision timeout.Default 2000. (n * 10 mSec)
#define TGAP_CONN_EST_LATENCY                   14  // 建立连接的从设备延迟（默认值：0）  //!< Link Layer connection slave latency.Default 0. (in number of connection events)
...
#define DEVDISC_MODE_NONDISCOVERABLE            0x00  // 不作设置  //!< No discoverable setting
#define DEVDISC_MODE_GENERAL                    0x01  // 扫描通用可发现设备  //!< General Discoverable devices
#define DEVDISC_MODE_LIMITED                    0x02  // 扫描有限的可发现设备  //!< Limited Discoverable devices
#define DEVDISC_MODE_ALL                        0x03  // 扫描所有  //!< Not filtered

// GAP_ADDR_TYPE_DEFINES GAP Address Types
#define ADDRTYPE_PUBLIC                         0x00  //!< Use the BD_ADDR
#define ADDRTYPE_STATIC                         0x01  // 静态地址  //!< Static address
#define ADDRTYPE_PRIVATE_NONRESOLVE             0x02  // 不可解析私有地址  //!< Generate Non-Resolvable Private Address
#define ADDRTYPE_PRIVATE_RESOLVE                0x03  // 可解析的私有地址  //!< Generate Resolvable Private Address
...
#define GAPROLE_BD_ADDR                         0x304  // 设备地址  //!< Device's Address. Read Only. Size is uint8_t[B_ADDR_LEN]. This item is read from the controller.
#define GAPROLE_ADVERT_ENABLED                  0x305  //!< Enable/Disable Advertising. Read/Write. Size is uint8_t. Default is TRUE=Enabled.
#define GAPROLE_ADVERT_DATA                     0x306  // 广播数据，默认全 0。  //!< Advertisement Data. Read/Write. Max size is B_MAX_ADV_EXT_LEN. Default to all 0.
#define GAPROLE_SCAN_RSP_DATA                   0x307  // 扫描应答数据，默认全 0  //!< Scan Response Data. Read/Write. Max size is B_MAX_ADV_EXT_LEN. Defaults to all 0.
#define GAPROLE_ADV_EVENT_TYPE                  0x308  // 广播类型，默认可连接非定向广播  //!< Advertisement Type. Read/Write. Size is uint8_t.  Default is GAP_ADTYPE_ADV_IND.
...
#define GAPROLE_MIN_CONN_INTERVAL               0x311  // 最小连接间隔，范围：1.5ms~4s，默认 8.5ms。  //!< Minimum Connection Interval to allow (n * 1.25ms).  Range: 7.5 msec to 4 seconds (0x0006 to 0x0C80). Read/Write. Size is uint16_t. Default is 7.5 milliseconds (0x0006).
#define GAPROLE_MAX_CONN_INTERVAL               0x312  // 最大连接间隔，范围：1.5ms~4s，默认 8.5ms。  //!< Maximum Connection Interval to allow (n * 1.25ms).  Range: 7.5 msec to 4 seconds (0x0006 to 0x0C80). Read/Write. Size is uint16_t. Default is 4 seconds (0x0C80).
// v5.x
...
#define GAPBOND_PERI_PAIRING_MODE               0x400  // 配对的方式，默认为：GAPBOND_PAIRING_MODE_WAIT_FOR_REQ  //!< Pairing Mode: @ref  GAPBOND_PAIRING_MODE_DEFINES. Read/Write. Size is uint8_t. Default is GAPBOND_PAIRING_MODE_WAIT_FOR_REQ.
#define GAPBOND_PERI_MITM_PROTECTION            0x401  // 中间人（MITM）保护。默认为 0，关闭中间人保护  //!< Man-In-The-Middle (MITM) basically turns on Passkey protection in the pairing algorithm. Read/Write. Size is uint8_t. Default is 0(disabled).
#define GAPBOND_PERI_IO_CAPABILITIES            0x402  // I/O 能力，模默认为：GAPBOND_IO_CAP_DISPLAY_ONLY，即设备仅能现实。  //!< I/O capabilities.  Read/Write. Size is uint8_t. Default is GAPBOND_IO_CAP_DISPLAY_ONLY @ref GAPBOND_IO_CAP_DEFINES.
...
#define GAPBOND_PERI_BONDING_ENABLED            0x405  // 如启用，则在配对过程中请求绑定。默认为 0，不请求绑定。  //!< Request Bonding during the pairing process if enabled.  Read/Write. Size is uint8_t. Default is 0(disabled).
...
#define GAPBOND_PERI_DEFAULT_PASSCODE           0x407  // 默认的中间人保护密钥，范围：0-999999，默认为0  //!< The default passcode for MITM protection. size is uint32_t. Range is 0 - 999,999. Default is 0.
...
typedef struct
{
    ...
    uint16_t connHandle;   //!< Connection message was received on 此处为devHandle！！！
...
#define GAPROLE_INIT                        0       // 等待启动  //!< Waiting to be started
#define GAPROLE_STARTED                     1       // 初始化完成但是未未广播  //!< Started but not advertising
#define GAPROLE_ADVERTISING                 2       // 正在广播  //!< Currently Advertising
#define GAPROLE_WAITING                     3       // 设备启动了但是未广播，此时正在等待再次广播  //!< Device is started but not advertising, is in waiting period before advertising again
#define GAPROLE_CONNECTED                   4       // 连接状态  //!< In a connection
#define GAPROLE_CONNECTED_ADV               5       // 连接状态且在广播  //!< In a connection + advertising
#define GAPROLE_ERROR                       6       // 无效状态，若为此状态表明错误  //!< Error occurred - invalid state
...
typedef struct tag_rf_config
{
    uint8_t LLEMode;                  //!< BIT0   0=basic, 1=auto def@LLE_MODE_TYPE
                                      ...
                                      // LLE_MODE_BASIC：Basic 模式，发送或接受结束后进入空闲模式
                                      // LLE_MODE_AUTO：Auto 模式，在发送完成后自动切换至接收模式
                                      // LLE_MODE_EX_CHANNEL：切换至 Frequency 配置频段
                                      // LLE_MODE_NON_RSSI：将接收数据的第一字节设置为包类型
    uint8_t Channel;                  // RF 通信通道（0-39）  //!< rf channel(0-39)
    uint32_t Frequency;               // RF 通信频率（2400000KHz-2483500KHz），建议不使用超过 24 次位翻转且连续的 0 或 1 不超过 6 个  //!< rf frequency (2400000kHz-2483500kHz)
    uint32_t accessAddress;           //!< access address,32bit PHY address
    uint32_t CRCInit;                 // CRC 初始值  //!< crc initial value
    pfnRFStatusCB_t rfStatusCB;       // RF 状态回调函数  //!< status call back
    uint32_t ChannelMap;              // 频道图，每一位对应一个频道。位值为 1 代表频道有效，反之无效。频道按位递增且频道 0 对应第 0 位  //!< indicating  Used and Unused data channels.Every channel is represented with a
                                      //!< bit positioned as per the data channel index,The LSB represents data channel index 0
    uint8_t Resv;
    uint8_t HeartPeriod;              // 心跳包间隔，为 100ms 的整数倍  //!< The heart package interval shall be an integer multiple of 100ms
    uint8_t HopPeriod;                // 跳跃周期（T=32n*RTC 时钟），默认为 8  //!< hop period( T=32n*RTC clock ),default is 8
    uint8_t HopIndex;                 // 数据信道选择算法中跳频信道间隔值，默认为 17  //!< indicate the hopIncrement used in the data channel selection algorithm,default is 17
 ...
 * 延迟 time*625μs 执行 event 事件，调用一次循环执行，除非运行 tmos_stop_task 关掉。
 *
 ...
 * 主机指令
 * 当客户端支持的值大于属性协议默认 ATT_MTU 的值时，客户端使用此程序将 ATT_MTU 设
 * 置为两个设备均可支持的最大可能值
 *
